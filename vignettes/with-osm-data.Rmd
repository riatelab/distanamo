...
Distance cartograms work by comparing two set of homologous points: the source points and the image points.

In our case, we'll be looking to build a unipolar distance cartogram using only data from OpenStreetMap (OSM): we'll be using both geographic layers describing the territory and travel time data.

This highlights the use of several packages developed at [RIATE](https://riate.cnrs.fr), including :

- `mapsf` to create maps with `sf` objects,
- `osrm` for calculating distance-time matrices from OSM data,
- `maposm` to retrieve OSM data (this is a wrapper around `osmdata`),
...
Depending on the radius defined in the `get_city` function the result list contains either individual buildings (if r <= 5000)
...
We then need a set of destinations to compute the travel times to.

To do so, we sample roughly 200 points on the road network:
...
mf_map(start, pch = 23, col = "red", cex = 3, add = T)
...
points$durations_osrm <- mat$durations
```
...
private instance. You can replace `Sys.getenv('VH_HOSTNAME')` and `Sys.getenv('VH_PORT')` by the actual values you want
to use.
...
We can call the `summary` function to ...

```{r}
summary(pos_result)
```

We can also plot the resulting object to 
...
```{r}
circles <- dc_concentric_circles(pos_result, list(5, 10, 15, 20, 25, 30))
```


## Creating the distance cartogram

We first need to compute the bounding box of all the layers we want to deform using the interpolation grid:
...
If needed, we can add the various points and their corresponding travel times:
...
In the case of *unipolar* cartograms, as proposed here, reading is generally fairly straightforward, making it easy to highlight locations that are difficult (or at least more difficult than the average accessibility) to reach.
