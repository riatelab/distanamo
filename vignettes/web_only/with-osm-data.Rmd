...
- `maposm` to easily retrieve and map OSM data (this is a wrapper around `osmdata`),
- `distanamo` for creating distance cartograms.

## Installing and importing packages

`mapsf`, `valh`, and `osrm` are available on CRAN while `distanamo` and `maposm` are only available on [r-universe](https://riatelab.r-universe.dev/builds).

```{r, eval=FALSE}
install.packages(c("mapsf", "valh", "osrm"))
repos <-  c("https://riatelab.r-universe.dev", "https://cloud.r-project.org")
install.packages(c("distanamo", "maposm"), repos = repos)
```

Once installed, we can load the necessary packages:
...
lat <- mean(bb[2, ])
res <- om_get(c(lon, lat), r = 3000)
om_write(res, filename = "res.gpkg")
```

```{r, include=FALSE}
res <- om_read("res.gpkg")
```


We can see that the resulting object is a list of sf layers, we can map with `om_map()`: 
...
```{r}
om_map(res, title = "Sampled points")
...
```{r, bg = TRUE, fig.width=10,fig.height=10, fig.show='hold'}
par(mfrow = c(2, 2))
...
```{r}
om_map(deformed, title = "Unipolar accessibility")
mf_map(igrid$interpolated_grid, col = NA, border = "#940000", lwd = .1, add = TRUE)
mf_map(start, pch = 16, col = "red", cex = 2, add = TRUE)
mf_map(st_intersection(st_geometry(circles), st_union(st_buffer(igrid$interpolated_grid, 0))),
       col = "red", lty = 2, lwd = 2, add = TRUE)
for (i in 1:4) {
  text(
    st_bbox(circles$geometry[i])$xmax,
    st_coordinates(start)[2],
    paste(i * 10, "min"),
    col = "purple",
    font = 2
  )
